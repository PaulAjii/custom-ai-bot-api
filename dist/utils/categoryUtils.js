...
(function (DocumentCategory) {
    ...
    DocumentCategory["Commodities"] = "Commodities";
    DocumentCategory["Chemicals"] = "Chemicals";
    DocumentCategory["Services"] = "Services";
 ...
 */
export function identifyQuestionCategory(question) {
    // Convert to lowercase for case-insensitive matching
    const normalizedQuestion = question.toLowerCase();
    // Score each category based on keyword matches
    const scores = Object.entries(categoryKeywords).map(([category, keywords]) => {
        const matchCount = keywords.filter(keyword => normalizedQuestion.includes(keyword.toLowerCase())).length;
        return { category: category, score: matchCount };
    });
 ...
 */
export function identifyDocumentCategory(filename, content) {
    // Convert to lowercase for case-insensitive matching
    ...
    if (content) {
        ...
        const scores = Object.entries(categoryKeywords).map(([category, keywords]) => {
            ...
            return { category: category, score };
        });
        // Find the category with the highest score
        const topCategory = scores.sort((a, b) => b.score - a.score)[0];
 ...
 */
export function evaluateContextRelevance(context, question) {
    if (!context.length)
        return 0;
    ...
    const relevanceScores = context.map(doc => {
        ...
        const matchingKeywords = questionWords.filter(word => content.includes(word));
 ...
 */
export function validateAnswerQuality(answer, question) {
    ...
    const relevantTermsInAnswer = keyTerms.filter(term => answer.toLowerCase().includes(term.toLowerCase()));
 ...
 */
function isGreetingOrCasualInteraction(input) {
    const normalizedInput = input.toLowerCase().trim();
    // Check if input matches any common greeting patterns
    const isGreeting = commonGreetings.some(greeting => normalizedInput === greeting ||
    ...
    const isSimpleQuestion = simpleQuestionPatterns.some(pattern => normalizedInput.includes(pattern));
 ...
 */
export function needsHumanHelp(question, context, answer) {
    ...
    const isComplexRequest = complexRequestIndicators.some(indicator => question.toLowerCase().includes(indicator));
 ...
 */
export function reorderDocumentsByRelevance(docs, question) {
    const keyTerms = extractKeyTerms(question);
    // Calculate relevance score for each document
